...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      subject(:my_milk) { described_class.new('milk') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_milk.type).to be('milk')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_milk.ounces).to be(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_water) { described_class.new('water') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(my_water).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_kombucha) { described_class.new('kombucha', 9) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_kombucha).not_to be_full
      end
    end
  end
end
