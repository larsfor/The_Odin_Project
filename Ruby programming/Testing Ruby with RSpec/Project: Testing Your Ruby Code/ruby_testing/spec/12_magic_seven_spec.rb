...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 6 and the random number is 3' do
      subject(:my_game) { described_class.new(3) }
      it 'should return 3' do
        previous_step = 6
        result = previous_step - subject.random_number
  ...
  describe '#play' do
    context 'when the random number is 99' do
      ...
      subject(:game) { described_class.new(99) }
      it 'will return 7' do
        expect(game.play).to be(7)
      end
    end

    context 'when the random number is 1337' do
      # remove the 'x' before running this test
      subject(:game) { described_class.new(1337) }
      it 'will return 7' do
        expect(game.play).to be(7)
      end
    end

    context 'when the random number is 99999999' do
      # remove the 'x' before running this test
      subject(:game) {described_class.new(99999999)}
      it 'will return 7' do
        expect(game.play).to be(7)
      end
    end
