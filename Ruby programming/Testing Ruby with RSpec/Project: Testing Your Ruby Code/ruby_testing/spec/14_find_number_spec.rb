...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    subject(:guess7) { described_class.new(5, 9, number_guessing) }
    context 'when min is 5 and max is 9' do
      it 'returns 7' do
        expect(guess7.make_guess).to eq(7)
      end
    end

    subject(:guess8) { described_class.new(8, 9, number_guessing) }
    context 'when min is 8 and max is 9' do
      it 'returns 8' do
        expect(guess8.make_guess).to eq(8)
      end
    end

    subject(:guess1) { described_class.new(0, 3, number_guessing) }
    context 'when min is 0 and max is 3' do
      it 'returns 1' do
        expect(guess1.make_guess).to eq(1)
      end
    end

    subject(:guess3) { described_class.new(3, 3, number_guessing) }
    context 'when min and max both equal 3' do
      it 'returns 3' do
        expect(guess3.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      subject(:game) { described_class.new(1, 2, random_number, 7) }
    ...
    context 'when guess and random_number are not equal' do
      # let(:random_number) { double('random_number', value: 7) }
      subject(:game) { described_class.new(1, 2, random_number, 8) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        ...
        expect(edge_case_game.min).to be(edge_case_game.max)
      end

      it 'does not update max' do
        old_max = edge_case_game.max
        edge_case_game.update_range
        expect(edge_case_game.max).to be(old_max)
      end
    end
  end
end
